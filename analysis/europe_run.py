...
d=np.load('data/football/europe_dominance.npz'); Y=d['Y']; N=len(Y); iu=np.triu_indices(N,1); off=~np.eye(N,dtype=bool)
rng=np.random.default_rng(700); hold=rng.uniform(size=len(iu[0]))<0.1
mask=np.ones((N,N)); mask[iu[0][hold],iu[1][hold]]=0; mask[iu[1][hold],iu[0][hold]]=0; H=(mask==0)&off; tr=(mask==1)&off
for f in sys.argv[1:]:
    R=res_load(); rec=R.get('europe_directed', dict(N=N, fits={}))
    ...
    R=res_load(); R['europe_directed']=rec; res_save(R); print(f, {k:(round(v,4) if isinstance(v,float) else v) for k,v in rec['fits'][f].items()}, flush=True)
